...
exp_strs = ['x2',
            '-kappa*x2 - w_0**2*x1 - w_J**2*( sin(x1 + phi_DC) - sin(phi_DC) - cos(phi_DC)*x1) + 2*kappa*(-amp_in*w_in*sin(w_in*t + phase_in) - 2*phi_max*w_in*cos(2*w_in*t))',
            'x2 - amp_in*w_in*sin(w_in*t + phase_in) + 2*phi_max*w_in*cos(2*w_in*t)',
            'r2',
...
params = [('w_0', 6.426939330532695e9 * 2 * np.pi),
          ('amp_in', [0, 0.01, 2]), # signal amplitude
          ('phi_max', [0, 1.3, 10]), # pump amplitude
...
kernel_input, kernel_output, kernel_body, kernel_op, shape = generate_kernel(var_strs, exp_strs, params, use_complex=False)
...
plt.figure()
pwrgain = 20*np.log10(amp[4,1,:,:,-1]/amp[4,1,0,:,-1])
plt.pcolor(omega_s/(2*np.pi), pwrgain, cmap='seismic')
plt.colorbar()
plt.clim([-20,20])
