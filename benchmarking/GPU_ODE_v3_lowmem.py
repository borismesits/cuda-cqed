...
def run_shots(variations1, variations2):
  ...
  def RK_loop(t, x, x_avg, x_var, f_dxdt, dt, args):
    ...
    for i in range(0, len(t)-1):
        ...
        k1 = f_dxdt(x, t[i], args)
        k2 = f_dxdt(x + k1*dt/2, t[i] + dt/2, args)
        k3 = f_dxdt(x + k2*dt/2, t[i] + dt/2, args)
        k4 = f_dxdt(x + k3*dt, t[i] + dt, args)

        x += (dt/6)*(k1 + 2*k2 + 2*k3 + k4)

    return x_avg, x_var, x
  ...
  def eom_i(args):
      ...
      x_avg, x_var, x = RK_loop(t, x0, x_avg, x_var, f_dxdt, dt, args)

      return x_avg, x_var, x, t
  ...
  def eom(args_list):
      ...
      x_avg, x_var, x, t = eom_i(args_list)
...
variations1 = [10, 30, 50, 100, 100, 300, 500, 1000, 1000]
variations2 = [10, 10, 20, 30,  100, 100, 200, 300,  1000]
