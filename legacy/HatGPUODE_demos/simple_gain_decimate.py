...
var_strs = ['a']
exp_strs = ['1j*(-1)*a*omega_0 + g*sin( (a+conjugate(a)) + phi_DC) + A_s*(1j)*(-1)*exp(-1j*(omega_s * t)) + A_p*(1j)*(-1)*exp(-1j*(2*omega_s * t + phi)) - kappa*a']
...
for i in range(0, shape[2]):
    plt.plot(t_d[1, -1, i, :], Q_demod[0, 1, -1, i, :])

# plt.figure()
# for i in range(0, shape[1]):
#     plt.plot(t_d[0, i, 25, :], Q_demod[0, i, 25, :])

plt.figure()
pwrgain = 20*np.log10(amp[0,1,:,:,-1]/amp[0,1,0,:,-1])
...
plt.figure()
plt.pcolor(np.log10(amp[0,0,:,:,-1]))
plt.title('Max log10 of signal w no pump: ' + str(np.log(np.max(amp[0,1,0,:,-1]))))
plt.colorbar()