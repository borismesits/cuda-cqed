...
'''
trying to more realistically add more modes

So this looks like a single paramp following a chi-shifted resonator, followed by a filter. "Circulation" is just one-way coupling
'''
...
params = [('omega_a', [10 * 2 * np.pi - chi/2, 10 * 2 * np.pi + chi/2, variations1]),
          ('omega_b', 10 * 2 * np.pi),
          ('omega_p', 20 * 2 * np.pi),
          ('omega_s', 10 * 2 * np.pi),
          ('A_p', 4.8),
